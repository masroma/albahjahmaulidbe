...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.kontakMitraBisni.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.kontak-mitra-bisnis.update", [$kontakMitraBisni->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('mitra_bisnis') ? 'is-invalid' : '' }}" name="mitra_bisnis[]" id="mitra_bisnis" multiple>
                    @foreach($mitra_bisnis as $id => $mitra_bisni)
                        <option value="{{ $id }}" {{ (in_array($id, old('mitra_bisnis', [])) || $kontakMitraBisni->mitra_bisnis->contains($id)) ? 'selected' : '' }}>{{ $mitra_bisni }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nama_kontak') ? 'is-invalid' : '' }}" type="text" name="nama_kontak" id="nama_kontak" value="{{ old('nama_kontak', $kontakMitraBisni->nama_kontak) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('jabatan') ? 'is-invalid' : '' }}" type="text" name="jabatan" id="jabatan" value="{{ old('jabatan', $kontakMitraBisni->jabatan) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('handphone') ? 'is-invalid' : '' }}" type="text" name="handphone" id="handphone" value="{{ old('handphone', $kontakMitraBisni->handphone) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="email" name="email" id="email" value="{{ old('email', $kontakMitraBisni->email) }}">
                @if($errors->has('email'))
