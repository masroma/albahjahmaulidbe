    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.kas-banks.update", [$kasBank->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('kode') ? 'is-invalid' : '' }}" type="text" name="kode" id="kode" value="{{ old('kode', $kasBank->kode) }}" required>
                @if($errors->has('kode'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('tipe_kas') ? 'is-invalid' : '' }}" name="tipe_kas" id="tipe_kas">
                    ...
                    @foreach(App\Models\KasBank::TIPE_KAS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('tipe_kas', $kasBank->tipe_kas) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('akun') ? 'is-invalid' : '' }}" name="akun" id="akun" required>
                    ...
                    @foreach(App\Models\KasBank::AKUN_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('akun', $kasBank->akun) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('akun'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nama') ? 'is-invalid' : '' }}" type="text" name="nama" id="nama" value="{{ old('nama', $kasBank->nama) }}" required>
                @if($errors->has('nama'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('mata_uang') ? 'is-invalid' : '' }}" name="mata_uang" id="mata_uang" required>
                    ...
                    @foreach(App\Models\KasBank::MATA_UANG_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('mata_uang', $kasBank->mata_uang) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('saldo') ? 'is-invalid' : '' }}" type="number" name="saldo" id="saldo" value="{{ old('saldo', $kasBank->saldo) }}" step="1">
                @if($errors->has('saldo'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tot_giro_keluar') ? 'is-invalid' : '' }}" type="number" name="tot_giro_keluar" id="tot_giro_keluar" value="{{ old('tot_giro_keluar', $kasBank->tot_giro_keluar) }}" step="1" required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('aktif') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="aktif" id="aktif" value="1" {{ $kasBank->aktif || old('aktif', 0) === 1 ? 'checked' : '' }}>
