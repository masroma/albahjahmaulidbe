...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.pajakItem.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.pajak-items.update", [$pajakItem->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('item') ? 'is-invalid' : '' }}" name="item_id" id="item_id" required>
                    @foreach($items as $id => $entry)
                        <option value="{{ $id }}" {{ (old('item_id') ? old('item_id') : $pajakItem->item->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('item'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pajak') ? 'is-invalid' : '' }}" name="pajak_id" id="pajak_id">
                    @foreach($pajaks as $id => $entry)
                        <option value="{{ $id }}" {{ (old('pajak_id') ? old('pajak_id') : $pajakItem->pajak->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('pajak'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('perhitungan') ? 'is-invalid' : '' }}" name="perhitungan" id="perhitungan">
                    ...
                    @foreach(App\Models\PajakItem::PERHITUNGAN_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('perhitungan', $pajakItem->perhitungan) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('tipe_pajak_item') ? 'is-invalid' : '' }}" name="tipe_pajak_item" id="tipe_pajak_item">
                    ...
                    @foreach(App\Models\PajakItem::TIPE_PAJAK_ITEM_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('tipe_pajak_item', $pajakItem->tipe_pajak_item) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
