...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.proyek.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.proyeks.update", [$proyek->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="kode">{{ trans('cruds.proyek.fields.kode') }}</label>
                <input class="form-control {{ $errors->has('kode') ? 'is-invalid' : '' }}" type="text" name="kode" id="kode" value="{{ old('kode', $proyek->kode) }}" required>
                @if($errors->has('kode'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nama') ? 'is-invalid' : '' }}" type="text" name="nama" id="nama" value="{{ old('nama', $proyek->nama) }}" required>
                @if($errors->has('nama'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pegawai') ? 'is-invalid' : '' }}" name="pegawai_id" id="pegawai_id">
                    @foreach($pegawais as $id => $entry)
                        <option value="{{ $id }}" {{ (old('pegawai_id') ? old('pegawai_id') : $proyek->pegawai->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('mitra_bisnis') ? 'is-invalid' : '' }}" name="mitra_bisnis_id" id="mitra_bisnis_id">
                    @foreach($mitra_bisnis as $id => $entry)
                        <option value="{{ $id }}" {{ (old('mitra_bisnis_id') ? old('mitra_bisnis_id') : $proyek->mitra_bisnis->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="status" id="status" value="1" {{ $proyek->status || old('status', 0) === 1 ? 'checked' : '' }}>
