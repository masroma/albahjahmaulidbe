...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.terminalKasir.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.terminal-kasirs.update", [$terminalKasir->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="kode_pos">{{ trans('cruds.terminalKasir.fields.kode_pos') }}</label>
                <input class="form-control {{ $errors->has('kode_pos') ? 'is-invalid' : '' }}" type="text" name="kode_pos" id="kode_pos" value="{{ old('kode_pos', $terminalKasir->kode_pos) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nama') ? 'is-invalid' : '' }}" type="text" name="nama" id="nama" value="{{ old('nama', $terminalKasir->nama) }}" required>
                @if($errors->has('nama'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('cabang') ? 'is-invalid' : '' }}" name="cabang" id="cabang">
                    ...
                    @foreach(App\Models\TerminalKasir::CABANG_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('cabang', $terminalKasir->cabang) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('gudang') ? 'is-invalid' : '' }}" name="gudang" id="gudang">
                    ...
                    @foreach(App\Models\TerminalKasir::GUDANG_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('gudang', $terminalKasir->gudang) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('kas_kasir') ? 'is-invalid' : '' }}" name="kas_kasir" id="kas_kasir">
                    ...
                    @foreach(App\Models\TerminalKasir::KAS_KASIR_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('kas_kasir', $terminalKasir->kas_kasir) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('kas_setor') ? 'is-invalid' : '' }}" name="kas_setor" id="kas_setor">
                    ...
                    @foreach(App\Models\TerminalKasir::KAS_SETOR_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('kas_setor', $terminalKasir->kas_setor) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('aktif') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="aktif" id="aktif" value="1" {{ $terminalKasir->aktif || old('aktif', 0) === 1 ? 'checked' : '' }}>
