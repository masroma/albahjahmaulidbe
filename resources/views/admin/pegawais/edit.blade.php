...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.pegawais.update", [$pegawai->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="kode">{{ trans('cruds.pegawai.fields.kode') }}</label>
                <input class="form-control {{ $errors->has('kode') ? 'is-invalid' : '' }}" type="text" name="kode" id="kode" value="{{ old('kode', $pegawai->kode) }}" required>
                @if($errors->has('kode'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nama') ? 'is-invalid' : '' }}" type="text" name="nama" id="nama" value="{{ old('nama', $pegawai->nama) }}" required>
                @if($errors->has('nama'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('alamat') ? 'is-invalid' : '' }}" type="text" name="alamat" id="alamat" value="{{ old('alamat', $pegawai->alamat) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kode_kota') ? 'is-invalid' : '' }}" name="kode_kota_id" id="kode_kota_id">
                    @foreach($kode_kotas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('kode_kota_id') ? old('kode_kota_id') : $pegawai->kode_kota->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('kode_pos') ? 'is-invalid' : '' }}" type="text" name="kode_pos" id="kode_pos" value="{{ old('kode_pos', $pegawai->kode_pos) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('telephone') ? 'is-invalid' : '' }}" type="text" name="telephone" id="telephone" value="{{ old('telephone', $pegawai->telephone) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('handphone') ? 'is-invalid' : '' }}" type="text" name="handphone" id="handphone" value="{{ old('handphone', $pegawai->handphone) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="email" name="email" id="email" value="{{ old('email', $pegawai->email) }}">
                @if($errors->has('email'))
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('aktif') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="aktif" id="aktif" value="1" {{ $pegawai->aktif || old('aktif', 0) === 1 ? 'checked' : '' }}>
