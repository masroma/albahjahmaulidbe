...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.cabang.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.cabangs.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nama_perusahaan_cabang') ? 'is-invalid' : '' }}" type="text" name="nama_perusahaan_cabang" id="nama_perusahaan_cabang" value="{{ old('nama_perusahaan_cabang', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('default_customer') ? 'is-invalid' : '' }}" name="default_customer_id" id="default_customer_id">
                    @foreach($default_customers as $id => $entry)
                        <option value="{{ $id }}" {{ old('default_customer_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
