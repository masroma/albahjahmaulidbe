...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.itemGroupOne.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.item-group-ones.update", [$itemGroupOne->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="kode">{{ trans('cruds.itemGroupOne.fields.kode') }}</label>
                <input class="form-control {{ $errors->has('kode') ? 'is-invalid' : '' }}" type="text" name="kode" id="kode" value="{{ old('kode', $itemGroupOne->kode) }}" required>
                @if($errors->has('kode'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nama') ? 'is-invalid' : '' }}" type="text" name="nama" id="nama" value="{{ old('nama', $itemGroupOne->nama) }}" required>
                @if($errors->has('nama'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('item_type') ? 'is-invalid' : '' }}" name="item_type" id="item_type" required>
                    ...
                    @foreach(App\Models\ItemGroupOne::ITEM_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('item_type', $itemGroupOne->item_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('akun_pembelian') ? 'is-invalid' : '' }}" name="akun_pembelian" id="akun_pembelian">
                    ...
                    @foreach(App\Models\ItemGroupOne::AKUN_PEMBELIAN_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('akun_pembelian', $itemGroupOne->akun_pembelian) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('akun_hpp') ? 'is-invalid' : '' }}" name="akun_hpp" id="akun_hpp">
                    ...
                    @foreach(App\Models\ItemGroupOne::AKUN_HPP_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('akun_hpp', $itemGroupOne->akun_hpp) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('akun_penjualan') ? 'is-invalid' : '' }}" name="akun_penjualan" id="akun_penjualan">
                    ...
                    @foreach(App\Models\ItemGroupOne::AKUN_PENJUALAN_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('akun_penjualan', $itemGroupOne->akun_penjualan) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('akun_retur_penjualan') ? 'is-invalid' : '' }}" type="text" name="akun_retur_penjualan" id="akun_retur_penjualan" value="{{ old('akun_retur_penjualan', $itemGroupOne->akun_retur_penjualan) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('tampil_pos') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="tampil_pos" id="tampil_pos" value="1" {{ $itemGroupOne->tampil_pos || old('tampil_pos', 0) === 1 ? 'checked' : '' }}>
