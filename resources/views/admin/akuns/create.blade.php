    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.akuns.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('akun_kode') ? 'is-invalid' : '' }}" type="text" name="akun_kode" id="akun_kode" value="{{ old('akun_kode', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('akun_nama') ? 'is-invalid' : '' }}" type="text" name="akun_nama" id="akun_nama" value="{{ old('akun_nama', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('akun_parent') ? 'is-invalid' : '' }}" name="akun_parent_id" id="akun_parent_id" required>
                    @foreach($akun_parents as $id => $entry)
                        <option value="{{ $id }}" {{ old('akun_parent_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('akun_jenis') ? 'is-invalid' : '' }}" name="akun_jenis_id" id="akun_jenis_id" required>
                    @foreach($akun_jenis as $id => $entry)
                        <option value="{{ $id }}" {{ old('akun_jenis_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('akun_klasifikasi') ? 'is-invalid' : '' }}" name="akun_klasifikasi_id" id="akun_klasifikasi_id" required>
                    @foreach($akun_klasifikasis as $id => $entry)
                        <option value="{{ $id }}" {{ old('akun_klasifikasi_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
