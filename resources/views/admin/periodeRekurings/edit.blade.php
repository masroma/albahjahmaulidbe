...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.periodeRekuring.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.periode-rekurings.update", [$periodeRekuring->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="kode">{{ trans('cruds.periodeRekuring.fields.kode') }}</label>
                <input class="form-control {{ $errors->has('kode') ? 'is-invalid' : '' }}" type="text" name="kode" id="kode" value="{{ old('kode', $periodeRekuring->kode) }}" required>
                @if($errors->has('kode'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('keterangan') ? 'is-invalid' : '' }}" type="text" name="keterangan" id="keterangan" value="{{ old('keterangan', $periodeRekuring->keterangan) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('faktor_pengali') ? 'is-invalid' : '' }}" name="faktor_pengali" id="faktor_pengali" required>
                    ...
                    @foreach(App\Models\PeriodeRekuring::FAKTOR_PENGALI_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('faktor_pengali', $periodeRekuring->faktor_pengali) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nilai_pengali') ? 'is-invalid' : '' }}" type="number" name="nilai_pengali" id="nilai_pengali" value="{{ old('nilai_pengali', $periodeRekuring->nilai_pengali) }}" step="1" required>
