...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.supplierItem.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.supplier-items.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('supplier_utamas') ? 'is-invalid' : '' }}" name="supplier_utamas[]" id="supplier_utamas" multiple>
                    @foreach($supplier_utamas as $id => $supplier_utama)
                        <option value="{{ $id }}" {{ in_array($id, old('supplier_utamas', [])) ? 'selected' : '' }}>{{ $supplier_utama }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('kode_barang_supplier') ? 'is-invalid' : '' }}" type="text" name="kode_barang_supplier" id="kode_barang_supplier" value="{{ old('kode_barang_supplier', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('lama_pemesanan') ? 'is-invalid' : '' }}" type="text" name="lama_pemesanan" id="lama_pemesanan" value="{{ old('lama_pemesanan', '') }}">
