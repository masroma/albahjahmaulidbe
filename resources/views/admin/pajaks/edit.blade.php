...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.pajak.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.pajaks.update", [$pajak->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="kode">{{ trans('cruds.pajak.fields.kode') }}</label>
                <input class="form-control {{ $errors->has('kode') ? 'is-invalid' : '' }}" type="text" name="kode" id="kode" value="{{ old('kode', $pajak->kode) }}" required>
                @if($errors->has('kode'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nama_pajak') ? 'is-invalid' : '' }}" type="text" name="nama_pajak" id="nama_pajak" value="{{ old('nama_pajak', $pajak->nama_pajak) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('akun_pembelian') ? 'is-invalid' : '' }}" name="akun_pembelian_id" id="akun_pembelian_id">
                    @foreach($akun_pembelians as $id => $entry)
                        <option value="{{ $id }}" {{ (old('akun_pembelian_id') ? old('akun_pembelian_id') : $pajak->akun_pembelian->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('akun_penjualan') ? 'is-invalid' : '' }}" name="akun_penjualan_id" id="akun_penjualan_id">
                    @foreach($akun_penjualans as $id => $entry)
                        <option value="{{ $id }}" {{ (old('akun_penjualan_id') ? old('akun_penjualan_id') : $pajak->akun_penjualan->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('presentasi_npwp') ? 'is-invalid' : '' }}" type="text" name="presentasi_npwp" id="presentasi_npwp" value="{{ old('presentasi_npwp', $pajak->presentasi_npwp) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('presentasi_non_npwp') ? 'is-invalid' : '' }}" type="text" name="presentasi_non_npwp" id="presentasi_non_npwp" value="{{ old('presentasi_non_npwp', $pajak->presentasi_non_npwp) }}">
