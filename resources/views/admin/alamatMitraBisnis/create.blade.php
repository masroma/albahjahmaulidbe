...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.alamatMitraBisni.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.alamat-mitra-bisnis.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('keterangan_alamat') ? 'is-invalid' : '' }}" type="text" name="keterangan_alamat" id="keterangan_alamat" value="{{ old('keterangan_alamat', '') }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('alamat_lengkap') ? 'is-invalid' : '' }}" name="alamat_lengkap" id="alamat_lengkap">{{ old('alamat_lengkap') }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kotas') ? 'is-invalid' : '' }}" name="kotas[]" id="kotas" multiple>
                    @foreach($kotas as $id => $kota)
                        <option value="{{ $id }}" {{ in_array($id, old('kotas', [])) ? 'selected' : '' }}>{{ $kota }}</option>
                    @endforeach
                </select>
                @if($errors->has('kotas'))
